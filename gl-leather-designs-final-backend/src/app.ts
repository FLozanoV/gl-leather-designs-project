...
import 'dotenv/config'; // Usar import para ES Modules
...
// --- MODIFICACIÓN CLAVE PARA EXTENDER INTERFACES DE EXPRESS ---
// Esto permite que TypeScript reconozca 'req.user' y 'req.file'
declare module 'express-serve-static-core' {
  interface Request {
    user?: { id: number; email: string; role: string }; // Define la estructura esperada del usuario decodificado
    file?: Express.Multer.File; // Usa el tipo correcto para multer.File
  }
}
...
const uploadPath = path.join(__dirname, '..', 'uploads'); // Directorio de destino para las imágenes
...
app.get('/check-db', asyncHandler(async (req: Request, res: Response) => {
...
app.post('/api/register', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [existingUserRows]: any = await connection.execute('SELECT id FROM users WHERE email = ?', [email]);
    ...
    const [result]: any = await connection.execute('INSERT INTO users (email, password, role) VALUES (?, ?, ?)', [email, hashedPassword, 'client']);
    const newUserId = result.insertId;

    return res.status(201).json({ message: 'Usuario registrado exitosamente', userId: newUserId });

  } catch (error: unknown) { // Tipado de 'error' a 'unknown'
...
app.post('/api/login', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [userRows]: any = await connection.execute('SELECT id, email, password, role FROM users WHERE email = ?', [email]);
    ...
    const token = jwt.sign(
      ...
      jwtSecret as string, // Usa la clave secreta cargada de .env
...
app.post('/api/products', protect, authorize(['admin']), upload.single('image'), asyncHandler(async (req: Request, res: Response) => {
  let connection: PoolConnection | undefined;
  try {
    ...
    if (!name || typeof name !== 'string' || name.trim() === '') {
      if (req.file) {
        fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar archivo fallido por validación (nombre vacío):', err); });
      }
    ...
    if (isNaN(parsedPrice) || parsedPrice <= 0) {
      if (req.file) {
        fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar archivo fallido por validación (precio inválido):', err); });
      }
    ...
    if (isNaN(parsedStock) || parsedStock < 0) {
      if (req.file) {
        fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar archivo fallido por validación de stock:', err); });
      }
    ...
    const [result]: any = await connection.execute('INSERT INTO products (name, description, price, stock, image_url, category) VALUES (?, ?, ?, ?, ?, ?)',
      [name.trim(), description ? description.trim() : null, parsedPrice, parsedStock, imageUrl, category]);
    
    const newProductId = result.insertId;
    const [rows]: any = await connection.execute('SELECT * FROM products WHERE id = ?', [newProductId]);

    if (rows.length === 0) {
      if (req.file) {
        fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar archivo tras fallo de recuperación del producto:', err); });
      }
    ...
    if (req.file) {
      fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar archivo tras fallo en la DB/ruta de creación:', err); });
    }
...
app.get('/api/products/:id', protect, asyncHandler(async (req: Request, res: Response) => { // <-- Se añade 'protect' si deseas que esta ruta también requiera autenticación
  let connection: PoolConnection | undefined;
  try {
...
app.put('/api/products/:id', protect, authorize(['admin']), asyncHandler(async (req: Request, res: Response) => {
  let connection: PoolConnection | undefined;
  ...
  try {
    ...
    const [productCheckRows]: any = await connection.execute('SELECT image_url, category FROM products WHERE id = ?', [productId]);
    ...
    const [updateResult]: any = await connection.execute(updateQuery, queryValues);
    ...
    const [updatedProductRows]: any = await connection.execute('SELECT * FROM products WHERE id = ?', [productId]);
...
app.patch('/api/products/:id/image', protect, authorize(['admin']), upload.single('image'), asyncHandler(async (req: Request, res: Response) => {
  let connection: PoolConnection | undefined;
  ...
  try {
    ...
    if (isNaN(productId)) {
      if (req.file) { // Si hay un archivo subido, elimínalo si el ID es inválido
        fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar archivo subido con ID inválido:', err); });
      }
      return res.status(400).json({ message: 'ID de producto inválido.' });
    }
    ...
    const [productCheckRows]: any = await connection.execute('SELECT image_url FROM products WHERE id = ?', [productId]);
    const existingProduct = productCheckRows[0];

    if (!existingProduct) {
      if (req.file) { // Si hay un archivo subido, elimínalo si el producto no existe
        fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar nuevo archivo subido para producto no existente:', err); });
      }
    ...
    if (updateResult.affectedRows === 0) {
      if (req.file) { // Si hay un archivo subido, elimínalo si no se actualizó ninguna fila
        fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar nuevo archivo no utilizado (no hubo affectedRows):', err); });
      }
    ...
    if (req.file) { // Asegurarse de eliminar la nueva imagen si falla la DB
      fs.unlink(req.file.path, (err) => { if (err) console.error('Error al eliminar archivo tras fallo en la DB (PATCH image):', err); });
    }
...
app.delete('/api/products/:id', protect, authorize(['admin']), asyncHandler(async (req: Request, res: Response) => {
  let connection: PoolConnection | undefined;
  try {
    const productId = parseInt(req.params.id, 10);
    if (isNaN(productId)) {
    ...
    const [productRows]: any = await connection.execute('SELECT image_url FROM products WHERE id = ?', [productId]);
    ...
    const [deleteResult]: any = await connection.execute('DELETE FROM products WHERE id = ?', [productId]);
    ...
    if (productToDelete.image_url) {
      // Usar path.basename para obtener solo el nombre del archivo de la URL
      const imageFileName = path.basename(productToDelete.image_url);
      const imagePath = path.join(uploadPath, imageFileName); 
...
app.use((err: Error, req: Request, res: Response, next: NextFunction) => {
  ...
  } else if ((err as any).code === 'ER_DUP_ENTRY') { // Si hay una propiedad 'code' (ej. de MySQL)
